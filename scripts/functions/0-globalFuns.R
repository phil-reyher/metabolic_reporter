...
regex_s <- function(df, x, y = 0, unit = "kg") {
  ...
  tmp <- sapply(df, function(col) {
    grep(x, col, perl = TRUE, ignore.case = TRUE)
  })
  ...
  ## else the value be left of the demographics' name
  if (grepl("weight", x)) {
    tmp_col <- grep(unit, tolower(df[tmp_row, ])) - 1
  } else if (y == "1") {
    tmp_col <- tmp_col - 1
  } else {
    tmp_col <- tmp_col + 1
...
findchangepts_meanvar <- function(x) {
  ...
  for (i in 3:(n - 2)) {
    ...
    for (j in 1:m) {
      ...
      mean1 <- mean(region1)
      mean2 <- mean(region2)
      log_likelihood1 <- sum(dnorm(region1,
        mean = mean1, sd = std1,
        log = TRUE
      ))
      log_likelihood2 <- sum(dnorm(region2,
        mean = mean2, sd = std2,
        log = TRUE
      ))
...
predict_work <- function(df, vo2_val) {
  ...
  model <- lm(work ~ vo2_abs_filt, data = df)
  new_observations <- data.frame(vo2_abs_filt = vo2_val)
...
save_plot_named_as_to <- function(plot, name, path) {
  ggsave(paste0(file.path(path, name), ".pdf"), plot,
    width = 297,
    height = 210, units = "mm"
  )
}
